...
var addMolecule = function(moleculeName, radiusScale, coorScale, translationVector){
	//var radiusScale = 0.04;
	//var coorScale = 4;
	var atomRadius, atomColor, atomCoor, newAtom;
	var objectArray = [];
	...
	for(var atomIndex = 0; atomIndex < moleculeName.length; atomIndex++){
		for(var visIndex = 0; visIndex < atomVisData.name.length; visIndex++){
			if(moleculeName[atomIndex][3] == atomVisData.name[visIndex]){
				atomRadius = atomVisData.radius[visIndex] * radiusScale;
				atomColor = atomVisData.color[visIndex];
				atomCoor = [moleculeName[atomIndex][0], moleculeName[atomIndex][1], moleculeName[atomIndex][2]];
				atomCoor = [atomCoor[0] - translationVector[0], atomCoor[1] - translationVector[1], atomCoor[2] - translationVector[2]];
				atomCoor = [atomCoor[0] * coorScale, atomCoor[1] * coorScale, atomCoor[2] * coorScale];
				newAtom = addAtom(atomCoor, atomRadius, atomColor);
				objectArray.push(newAtom);
			};
		};
	};
	return objectArray;
...
var groupMolecule = function(objectArray){
	var moleculeGroup = new THREE.Object3D();
	for(var objIdx = 0; objIdx < objectArray.length; objIdx++){
		moleculeGroup.add(objectArray[objIdx]);
	}
